...
public class GameManager : MonoBehaviour
{
    ...
    private List<Sprite> availableSprites = new List<Sprite>(); // Доступные спрайты
    private Sprite lastUsedSprite; // Последний использованный спрайт

  
    [SerializeField] private List<Sprite> guestSprites; // Добавляем список спрайтов
    ...
    void Start()
    {
        // Инициализируем список доступных спрайтов
    ...
    GameObject InstantiateGuest(Transform spot)
    {
        ...
        // Устанавливаем случайный уникальный спрайт
    ...
    Sprite GetUniqueRandomSprite()
    {
        ...
        {
            // Если все спрайты использованы, сбрасываем список
            availableSprites = new List<Sprite>(guestSprites);

            // Удаляем последний использованный спрайт, чтобы избежать повторений
